...
Codespace の作成時に、a-blog cms 本体一式が**ワークスペースの `acms/` フォルダに自動展開**されます。エディタで `acms/` 配下（`acms/themes/` のテーマ、`acms/extension/plugins/` のプラグイン、コアの PHP など）を編集すると、**そのままブラウザの表示に反映**されます。

- 例: `acms/themes/` に自分のテーマを追加してウィザードで選択、`acms/extension/plugins/` に独自プラグインを配置、など。
- `acms/` は使い捨て・**Git 管理外**（`.gitignore` 済み。a-blog cms は商用ソフトのため本体はコミットしません）。Codespace を削除すると消えます。残したい成果物は別途エクスポート／コミットしてください。
...
cd acms-codespaces
bash .devcontainer/setup.sh   # a-blog cms 本体を ./acms へ展開（初回のみ・要 Docker 起動）
docker compose up
# ブラウザで開く:
...
| ファイル | 役割 |
| --- | --- |
| `compose.yaml` | `appleple/acms`（Apache + PHP 同梱）と `mysql:8.0` を定義。acms はホスト側 `8080` に公開し、`acms/` を docroot にマウント。 |
...
| `.devcontainer/setup.sh` | 初回に a-blog cms 本体をイメージから `acms/` へ展開する（編集可能にするため）。 |
| `acms/` | 展開された a-blog cms 本体（編集対象）。`.gitignore` 済み・使い捨て。 |
